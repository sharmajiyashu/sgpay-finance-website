...
import { AGENT_API_PATHS, API_CONFIG } from "@/lib/config/env";

const BASE_URL = API_CONFIG.agent;

const TOKEN_EXPIRY_MS = 6 * 60 * 60 * 1000;
const TOKEN_KEY = "sgAgentToken";
const TOKEN_EXPIRY_KEY = "sgAgentTokenExpiry";
const AUTH_USER_KEY = "sgAgentAuthUser";
...
function toErrorMessage(value: unknown, fallback = "Request failed"): string {
  ...
  return String(value).trim().length > 0 ? String(value) : fallback;
}
...
api.interceptors.response.use(
  (response) => {
    ...
    if (wrapped && typeof wrapped.success === "boolean" && !wrapped.success) {
      return Promise.reject(new Error(toErrorMessage(wrapped.error, "Request failed")));
    }
    return response;
  },
  (error) => {
...
export function setToken(token: string): void {
  if (typeof window !== "undefined") {
    ...
    localStorage.setItem(TOKEN_EXPIRY_KEY, String(Date.now() + TOKEN_EXPIRY_MS));
  }
}
...
export { AGENT_API_PATHS, API_CONFIG };
